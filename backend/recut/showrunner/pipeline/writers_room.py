...
def _writer_sys(register: str) -> str:
    return (
        ...
        "Rules: 2-4 characters each with a clear want AND flaw; subtext over on-the-nose. "
        "SCENES = LOCATION/TIME JUMPS, nothing else: a story that is ONE continuous "
        "conversation in ONE place is ONE scene (its beats live inside it) — never split "
        "a conversation across scenes (each scene boundary costs a visual transition and "
        "risks continuity drift). Use 2-3 scenes ONLY when the story truly moves. "
